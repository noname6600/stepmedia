 ...
 */
@RequestMapping("/")
public interface ISearchInfoController<I, T extends IBaseData<I>, U extends T, F extends ISearchFilter> extends IPagingFactoryController<I, T, U> {
    ...
    @GetMapping("/search")
    default ResponseEntity<BaseResponse<BasePagingResponse<T>>> searchInfo(
            @ParameterObject @Valid F filter,
            @RequestParam(name = "number", defaultValue = "0") Integer number,
            @RequestParam(name = "size", defaultValue = "20") Integer size
    ) {
        return factoryGetSearchInfoWithFilter(filter, number, size);
    }
