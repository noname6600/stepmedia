...
const CSSTransition = ({
  ...
  const addClasses = (node: HTMLElement | undefined | null, classes: string[]) => {
    if (node != null && classes.length) {
      node.classList.add(...classes);
    }
  };

  const removeClasses = (node: HTMLElement | undefined | null, classes: string[]) => {
    if (node != null && classes.length) {
  ...
  return (
    <ReactCSSTransition
      ...
      onEnter={() => {
        if (!removeFromDom && nodeRef.current) nodeRef.current.style.display = 'flex';
